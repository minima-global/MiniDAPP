...
var Minima = {
	...
	/**
	 * The CodeID required to logon to the RPC	
	 */
	minicodeid : "0x00",
	...
	init : function(callback){
		...
		if(startid!=-1 && endid!=-1){
			...
			//And now get the CodeID..
			var endcodeid = Minima.form.getParams("minicodeid");
			if(endcodeid == null){
				Minima.notify("MINDAPP CODEID MISSING!<br><br>MiniDAPP won't work..", '#BEB25B');
			}else{
				Minima.minicodeid = endcodeid;
				Minima.log("MINDAPP CODEID : "+endcodeid);
			}
...
//	httpPostAsync(Minima.rpchost+"/"+type+"/"+Minima.minicodeid+"/"+Minima.minidappid, encodeURIComponent(data), callback);
//}
function MinimaRPC(type, data, callback){
	//Create a JSON for the message
	var jsonrpc = {};
	jsonrpc.minidappid = ""+Minima.minidappid;
	jsonrpc.minicodeid = ""+Minima.minicodeid;
	jsonrpc.type 	   = type; 
	jsonrpc.command    = data;

	//Stringify..
	var jsonrpcstr = JSON.stringify(jsonrpc);
	
	//Encode..
	var encmessage = encodeURIComponent(jsonrpcstr);
	
	//And now fire off a call 
	httpPostAsync(Minima.rpchost+"/jsonrpc/", encmessage, callback);
}
        ...
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200){
        	...
        	if(callback){
        		try {
					...
					Minima.log("ERROR    : "+theUrl+" "+params);
					Minima.log("RESPONSE : "+xmlHttp.responseText);
