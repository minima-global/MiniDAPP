...
var Minima = {
	...
	//Unique ID for this instance so that messages are routed correctly
	...
	net : {
		...
		listen : function(port,callback){
			MinimaRPC("net","listen "+port,callback);
		},
		
		stop : function(port,callback){
			MinimaRPC("net","stop "+port,callback);
		},
		
		broadcast : function(jsonobject, port, callback){
			MinimaRPC("net","broadcast "+port+" "+JSON.stringify(jsonobject),callback);
		},
		
		connect : function(hostport, callback){
			MinimaRPC("net","connect "+hostport,callback);
		},
